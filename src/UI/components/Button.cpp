...
void Button::draw(sf::RenderWindow& window) {
    ...
    if (!enabled_) {
        ...
        sf::Color fill = background_.getFillColor(); 
        sf::Color outline = background_.getOutlineColor();
        sf::Color foreground = text_.getFillColor();

        // Chỉ đổi màu nếu có Style đặc biệt hoặc được Selected
...
void Button::setCustomColor(sf::Color fill, sf::Color outline, sf::Color textCol) {
    background_.setFillColor(fill);
    background_.setOutlineColor(outline);
    text_.setFillColor(textCol);
} // Hân thêm
